...
static int		isolate(char **line, t_list *cur)
{
	...
	if (!*(char *)cur->content)
		return (0);
	if (!(next_line = ft_strchr((char *)cur->content, '\n')))
	{
		if (!(*line = ft_strdup((char *)cur->content)))
			return (-1);
		free(cur->content);
		if (!(cur->content = (void *)ft_strnew(0)))
			return (-1);
	}
	else
	{
		if (!(*line = ft_strnew(next_line - (char *)cur->content)))
			return (-1);
		ft_strncpy(*line, (char *)cur->content, next_line -
		(char *)cur->content);
		ft_strcpy((char *)cur->content, next_line + 1);
	}
	return (1);
}
...
static int		read_fd(int fd, t_list *cur)
{
	...
	if (!(cur->content = (void *)ft_strjoin(cur->content, buf)))
	{
		free((void *)buf);
		return (-1);
	}
...
static t_list	*fd_find(int fd, t_list *list, t_list **root)
{
	...
	if (!list)
	{
		if (!(tmp = ft_lstnew_empty()))
			return (0);
		if (!(tmp->content = (void *)ft_strnew(0)))
			return (0);
